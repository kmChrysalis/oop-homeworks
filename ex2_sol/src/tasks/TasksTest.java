...
// Testing tasks (question 2)
	...
	private static void checkDep(int[] a, int task1, int task2, String msg) {
	...
	private static void testTasks() {
		...
		int[] order = t.order();
		checkDep(order, 1, 3, "Tasks(1)");
		checkDep(order, 3, 5, "Tasks(2)");
		checkDep(order, 5, 2, "Tasks(3)");
	}

	private static boolean fulfills(String[] a, String task1, String task2) {
		for (String s : a) {
			if (s.equals(task1))
				return false;
			if (s.equals(task2))
	...
	private static void checkDep(String[] a, String task1, String task2, String msg) {
		check(fulfills(a, task1, task2),
				msg + ": Answer " + Arrays.toString(a)
						+ " does not respect: " + task1
						+ " dependes on " + task2);
