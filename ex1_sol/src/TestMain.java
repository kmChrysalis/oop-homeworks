...
public class TestMain {
	...
	private static void checkEq(Object a, Object b, String message) {
		check(a.equals(b), String.format("%s: Expected \"%s\", but got \"%s\"", message, b, a));
	}
	...
	private static void checkPublicMethods(Class<?> c, String[] methods) {
		...
		for (Method m : Object.class.getMethods())
			s.add(m.getName());
		s.addAll(Arrays.asList(methods));
	...
	private static void testDeck() {
		...
		Deck d1 = new Deck(3);
		checkEqStr(d1, "[0C, 0D, 0H, 0S, 1C, 1D, 1H, 1S, 2C, 2D, 2H, 2S]","Deck.Deck(int)");
		Deck d2 = new Deck(d1, 4);
		checkEqStr(d2, "[2S, 2H, 2D, 2C]", "Deck.Deck(Deck, int)");
		Deck d3 = new Deck(d1, d2);
		checkEqStr(d3, "[1S, 2C, 1H, 2D, 1D, 2H, 1C, 2S, 0S, 0H, 0D, 0C]","Deck.Deck(Deck, Deck)");
		check(d1.getNumCards() == 0 && d2.getNumCards() == 0,"Deck.Deck(Deck, Deck) not emptying parameters");
		d3.sort();
		checkEqStr(d3, "[0C, 0D, 0H, 0S, 1C, 1D, 1H, 1S, 2C, 2D, 2H, 2S]","Deck.sort");
